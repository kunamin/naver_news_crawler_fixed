...
def crawler(maxpage, query, sort, s_date, e_date):
    ...
    while page <= maxpage_t:
        url = "https://search.naver.com/search.naver?where=news&query=" + (
                query) + "&sm=tab_opt&sort=" + sort + "&photo=0&field=0&reporter_article=&pd=3&ds=" + s_date + "&de=" + e_date + "&docid=&nso=so:da,p:from" + (
                s_from) + "to" + e_to + ",a:all&mynews=0&start=" + str(page) + "&refresh_start=0"
        ...
        # <a>태그에서 제목과 링크주소 추출
        atags = soup.select('.news_area > a')
        for atag in atags:
            title_text.append(atag.text)  # 제목
            link_text.append(atag['href'])  # 링크주소

        # 신문사 추출
        source_lists = soup.select('.info_group > a')
        for source_list in source_lists:
            if source_list.text != "네이버뉴스":
...
def main():
    ...
    query = input("검색하고 싶은 검색어를 입력하세요: ")
